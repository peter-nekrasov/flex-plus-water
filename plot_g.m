...
beta = 0.5; %1.1 + 1i*0.0001;
gamma = -2; %-(1.1)^(5/4)*((1/5)^(1/4)-(1/5)^(5/4));
d1 = -(beta);
d0 = gamma;
...
for i = 1:5
    rhoj = rts2(i);
    ej = ejs(i);
    G1 = G1 + ej*rhoj^2*pi/2*StruveH0Y0(-rhoj*xs);
    if imag(rhoj) == 0
        [struveR, ~] = struve102(rhoj*xs);
        G2 = G2 + 1i*ej*rhoj^2*(struveR + besselh(0,rhoj*xs));
    else
        [struveK, ~] = struveK102(-rhoj*xs);
        G2 = G2 + ej*rhoj^2*pi/2*struveK;
    end
end


G = G1;
plot(xs,real(G), xs, imag(G))
