function errs = get_fin_diff_err3(X,Y,mu,phi_n,phi,h,coefs,xlocs,ylocs)
...
for kk = 1:numel(xlocs)

    xloc = xlocs(kk);
    yloc = ylocs(kk);

    ind = find((X == xloc).*(Y == yloc));
    ...
    gterm = g0.*phi(ii,jj);
    errs(kk) = abs(first + second + third + fourth + fifth + sixth + seventh + bterm + gterm) / max(abs([first second third fourth fifth sixth seventh bterm gterm])); % divide by largest term
