function err = get_fin_diff_err(X,Y,mu,phi_n,phi,h,coefs,xloc,yloc)
    ...
    ind = intersect(find(abs(X - xloc)<1e-12), find(abs(Y - yloc)<1e-12));
    ...
    gterm = g0.*phi(ii,jj);
    err = abs(first + second + third + fourth + fifth + sixth + seventh + bterm + gterm) / max(abs([phi(:); phi_n(:); first; second; third; fourth; fifth; sixth; seventh; bterm; gterm]));
