...
muconv = load('mu2.mat').mu;

[Xerr,Yerr] = meshgrid(-4:0.5:4);
...
for ii = 1:numel(hs)
    ...
    [coefs, H] = bump3(X,Y,2,4,2); % remove gbar from coefs vector
    ...
    if h > hsmall     % Interpolating the density
        ...
        tmpinterp = mu_interp(1:3585,1:3585);
    ...
    [coefs, H] =  bump3(Xsmall,Ysmall,2,4,2);
...
saveas(gcf,'convergence2.fig','fig')
exportgraphics(gcf,'convergence2.pdf','ContentType','vector')