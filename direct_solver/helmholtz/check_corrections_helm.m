...
for ii = 1:numel(hs)
    ...
    [X,Y] = meshgrid(-5:h:5);
    src = [X(:).'; Y(:).'];
    kern = kernmat(src,targ,@(s,t) helm2d.green_cell_helm(zk,s,t),h);
    val = kern{1};
    d1 = dens(X,Y);
    dint = sum(val(:).*d1(:),'all');
    ...
    kern = kernmat(src,targ,@(s,t) helm2d.green_cell_helm(zk,s,t),h,inds,corrs);
    val = kern{1};
    d1 = dens(X,Y);
    dint = sum(val(:).*d1(:),'all');
...
hold on
legend('Without corrections','With corrections','h^2','h^4','h^6','h^8')